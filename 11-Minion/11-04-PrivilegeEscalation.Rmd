...
#### Secuestro de commandos para copiar los ficheros del usuario decoder.MINION {-}
...
Analizando con el commando type lo que hacen estos script, vemos que el `del_logs.bat` llama al fichero `c.ps1` y lo
...
Tambien vemos un fichero `backup.zip` y si le checkeamos por **Aditionnal Data Streams** con el commando
...
Vemos que tiene un stream llamado pass. Lo miramos con el commando `type`

```bash
type C:\Temp\backup.zip:pass
```

y encontramos un hash. Si lo passamos por [crackstation](https://crackstation.net/) nos da la contraseña.

#### Ejecucion de commandos como administrator con ScriptBlock {-}

Aqui el problema es que no tenemos connectividad con **smb** o otros puertos para connectarnos como root. La idea
aqui seria de ejecutar commandos como administrator para cambiar la reglas del Firewall.
...
Aqui vemos que podemos ejecutar commando como el usuario administrator. Vamos a por el cambio en el firewall
...
Si ahora desde la maquina de attackante le hacemos un nmap para ver los puertos abiertos

```bash
nmap -sS --min-rate 5000 --open -vvv -n -Pn -p- 10.10.10.57
```

vemos que tenemos todo expuesto y como hay el puerto 3389 que es el puerto **RDP** ya nos podemos connectar con Remmina por ejemplo.
