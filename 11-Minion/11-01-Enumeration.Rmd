...
#### Checkear la web por puerto 62696 {-}
...
```bash
wfuzz -c -t 200 --hc=404 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt http://10.10.10.57:62696/WFUZZ
```

Encontramos un ruta `/backend` pero no se ve nada en firefox. Decidimos fuzzear con la extension `.asp`

```bash
wfuzz -c -t 200 --hc=404 -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt http://10.10.10.57:62696/WFUZZ.asp
```

Aqui encontramos un fichero `test.asp` y navigando no dice que no encuentra el parametro `u` que tendria que ser un URL.
Intentamos ver si se connecta a nuestro servidor web
...
1. Intentamos connectar por la web 
...
Aqui no passa nada. La idea aqui, como solo tiene un puerto avierto seria de explorar si tiene puerto privados usando localhost
...
Esto functiona. Pero no vemos en la web el output del commando. Solo vemos el codigo de estado (0 si el commando a functionado, 1 si no a functionado)
