...
Tambien se podri utilizar la heramienta creada por s4vitar [ttyoverhttp](https://github.com/s4vitar/ttyoverhttp)

### Analyzando el servidor {-}
...
El commando `ls -l /home/xalvas` nos muestra ficheros. En el fichero `intrusions` vemos lo siguiente
...
Vemos que el commando `nc` esta BlackListeado y que loggea el Proccess Kill en este fichero. El problema de esto es que se puede
que los commandos BlackListeados se controllan con los nombres mismo (no permite `nc, python, bash`). Pero que passa si copiamos el 
binario bash y que le ponemos un nombre differente.

1. Nos ponemos en escucha por el puerto 443 en la maquina de attaquante
...
Ya lo podemos lanzar con el commando `python3 autopwn.py`
...
Hay otros ficheros de typo `.wav`, usando la misma technica nos lo enviamos tambien.
Checkeamos que los ficheros no sa hayan compormetido durante el transfer con `md5sum`. Los ficheros `.wav` son ficheros
de typo audio y se pueden escuchar con el commando `play recov.wav`
...
### Reto de steganografia con Audoacity {-}

**Audacity** es una heramienta de audio que se puede installar con `apt install audacity`. Lo abrimos y cargamos los dos ficheros.
Si nos dan 2 audios que parrecen se los mismos pero hemos visto con el **md5sum** que no son iguales, Una cosa que se puede hacer es 
