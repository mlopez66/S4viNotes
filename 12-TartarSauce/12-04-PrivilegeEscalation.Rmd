...
Aqui vemos que no podemos hacer nada y que no tenemos possiblidad de rootear la maquina por vulnerabilidades del proprio usuario.
Tenemos que enumerar el systema.
...
Bueno aqui no se ve nada, no tenemos permissios SUID no hay nada vemos tareas cron. Pero siempre se puede ver de forma alternativa si hay tareas 
...
Aqui nos creamos el script que nos servira de monitoring de processos.
...
Ya podemos ver que hay una tarrea `/bin/bash /usr/sbin/backuperer` que se ejecuta a interval regular de tiempo. lo Analizamos.
...
otro comprimido. Como supponemos que es **root** que ejecuta la tarea, podemos approvechar de esto para ver la flag de root.
...
1. Preparamos en la maquina de attackante para recivir el comprimido
...
Ahora que tenemos el comprimido en la maquina de attackante, vamos a cambiar su contenido
...
Cuando la pantalla nos muestre el mensaje `[+] Archivo hijackeado con exito`, podemos mirrar el fichero `/var/backup/onuma_backup_error.txt` 
